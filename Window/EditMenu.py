...
def open_edit_menu(window_x, window_y, db_dict, attr_dict=None, attr_list=None, settings=None):
    ...
    def start_btn_func():
        ...
        if settings['mode'] == 'players':
            ...
            if edit_type_radio_group.value == 'add':

                db_players = db_dict['player_db'][1]
                search_results = db_players.search(attr_dict)
            ...
            # Get players from list to edit or delete and search
                ...
                search_results = list_players.search(attr_dict)
    ...
    def display_attributes():
        ...
        for sort_item in settings['messages']['sort']:
            view.add(sort_item)
        for sort_item in settings['messages']['results']:
            view.add(sort_item)
    ...
    def player_bio_btn_func(player):
        win_edit.hide()
        PlayerBio.open_player_bio_window(win_edit.x, win_edit.y, player, db_dict, win_edit)
        win_edit.become_target()
    ...
    attribute_btn.y = simple_btn.bottom + 5
    ...
    asc_desc_rg_msg.y = attribute_btn.bottom + radio_btn_space
    advanced_display.append(asc_desc_rg_msg)
    ...
    advanced_display.append(descend_radio_btn)
    ...
    advanced_display.append(ascend_radio_btn)
    ...
    add_to_list_btn = Button("Add All Players")
